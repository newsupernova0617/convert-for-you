...
```
.
├── backend/        # NestJS API for job creation, status polling, and queue orchestration
├── frontend/       # Next.js + Tailwind marketing site and conversion dashboard
├── README.md       # Product blueprint and contributor guide
└── text.txt        # Playground file (unused)
```
...
By default the frontend proxies `/api/jobs` requests to `http://localhost:4000`. Override with `CONVERT_API_URL` in `.env.local`.
...
Environment variables are defined in `backend/.env.example`. The development server exposes REST endpoints:

| Method | Path       | Description                         |
| ------ | ---------- | ----------------------------------- |
| POST   | `/jobs`    | Queue a new conversion request.     |
| GET    | `/jobs/:id`| Fetch the latest status for a job.  |

Uploaded files are stored in memory and processed by a simulated worker loop that updates job status. Replace the in-memory queue and mock processor with BullMQ workers + object storage when connecting real converters (LibreOffice, ImageMagick, Tesseract, etc.).
