...
**Special handling with image optimization**:

1. Write PDF to temp file
2. LibreOffice converts PDF to PNG (only first page)
3. Use Sharp library for optimization:
...
```javascript
// JPG optimization
sharp(pngBuffer).jpeg({ quality: 90, progressive: true }).toBuffer()

// PNG optimization
sharp(pngBuffer).png({ compressionLevel: 9 }).toBuffer()
```
...
| JPG (image) | 0.1-0.5 MB | First page only |
| PNG (image) | 0.2-1 MB | Lossless compression |

---

Generated by Claude Code | Last Updated: 2024-11-04
